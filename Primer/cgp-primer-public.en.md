...
approach from personal-use CGP Section 3.1 (verify through execution, and
actively try to break it rather than merely confirming it works) be
applied to the post-resolution implementation.

- Restrict the instruction to the AI to: **"Try to break this
  implementation and show me whether it correctly reflects the finalized
  decision. Don't just confirm it works as expected — actively hunt for
  inputs and edge cases where it behaves differently than expected."**
